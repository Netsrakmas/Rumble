...
2. **Jump** — deliberately modest (~2.6 tiles), variable height, coyote 120 ms, buffer 120 ms, corner correction 8 px, apex gravity 0.5×, fall gravity 1.7× rise.
...
jumpVy=-300  releaseGravMult=1.9  jumpHBoost=30
coyote=0.120  jumpBuffer=0.120  cornerCorrection=8
...
gravity multiplier (`releaseGravMult`, Pittman model; 2.5 at first, softened to 1.9 after playtest feedback "jump too hard to time" — max held height unchanged, gun-lift now hold-agnostic via a 0.25s grace) instead of the
