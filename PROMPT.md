...
- [ ] **M1 — Vertical slice:** loop, input, collision, full jump feel (coyote/buffer/variable/corner), camera, one test room, placeholder player rendering via manifest. Gate: Playwright movement tests pass, zero console errors.
- [ ] **M2 — Full verb set:** shoot, gun-jump, charge, melee, roll, crawl, wall slide/jump, projectiles. Gate: criterion 3 + all verbs demonstrable in test room.
- [ ] **M3 — Systems:** health/damage/i-frames/death/respawn, checkpoints + localStorage, tickets, vending machine, ability flags/gates, enemies (weevil/gnat/sporespitter), HUD. Gate: full loop damage→death→respawn verified.
- [ ] **M4 — Content:** Level 1's six rooms authored as data, Bullhorn Beetle boss, Level 2 stub, door transitions, level index. Gate: full playthrough start→Level 2, loader validation tests.
- [ ] **M5 — Polish:** parallax sandwich, all particles, shake/hit-stop everywhere, squash-stretch, audio set, title screen, room-transition easing, vignette, prompts. Gate: forbidden-list audit + screenshot squint test vs style bible.
- [ ] **M6 — Verification:** full Playwright harness (test skill), perf sample, criteria 1–8 all green. Gate: harness green twice consecutively.
